...
description: Manage an Obsidian knowledge-base vault through the official Obsidian CLI, following the target vault's own folder, property, tag, link, template, archival, and configured Git synchronization conventions.
---

# Obsidian CLI Knowledge Base

Use this skill whenever the user asks to read, search, create, edit, classify, move, rename, archive, review, or automate notes in the configured Obsidian vault.

## CLI-first rule

- Prefer the official Obsidian CLI for all note operations: `read`, `search`, `search:context`, `create`, `append`, `prepend`, `move`, `rename`, `open`, `property:set`, `property:remove`, `properties`, `tags`, `tasks`, `links`, `backlinks`, `unresolved`, `orphans`, `deadends`, `daily`, and `templates`.
...
The Git behavior may be configured per vault by its own policy note or by an explicit user request. The default for successful note mutations remains commit and push; the vault's own conventions still control note structure, metadata, links, and folders.
...
These rules reduce duplicate summaries, preserve a single source of truth, and make the vault easier to navigate without forcing links where no meaningful relationship exists.
...
3. Fill in the content first, then add only the properties, tags, and links supported by the vault's rules and useful for retrieval.
4. Verify the created path, frontmatter, and links.

### Archiving

1. Inspect the note and confirm it is complete, inactive, or obsolete.
2. Move it with CLI to the vault's archive location, if one exists.
3. Apply the vault's archive status or metadata while retaining content, provenance, tags, and useful links.
4. Re-read the moved file and check for broken links.

### Review

Use the CLI to inspect `tasks todo verbose`, `unresolved verbose`, `orphans total`, `deadends total`, `tags counts`, and `properties counts`. Treat the results as signals for review, not automatic cleanup instructions.
